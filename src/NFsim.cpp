 ...
 * that depend on local context.  NFsim is designed to operate with the
 * BioNetGen Language (http://bionetgen.org/).  The new version of BNG is able
 * to generate an XML encoded form of the BNG Language, which NFsim can take as
 * input.
 *
 * For more details on setting up, running, and getting output from an NFsim
 * simulation see the User Manual.  The User Manual also has additional
 * information for new developers.  The manual is available online along with
 * examples here: http://www.nfsim.org
 *
 *
 *
 * \section key Command Line Argument List
 *
 *  Arguments can be provided to NFsim through the command line.  Below is a
 * partial list of the available commands and a brief description of what they
 * do.  For more details, see the NFsim user manual.
 *
 ...
 *
 *  -oSteps [num of steps] = specifies the number of times to output during the
 * simulation
 *
 *  -eq [Duration in sec] = specifies the length of time to equilibrate before
 * simulating
 *
 ...
 *
 *  -gml [integer] = sets maximal number of molecules, per any MoleculeType, see
 * manual
 *
 *  -nocslf = disable evaluation of Complex-Scoped Local Functions
 *
 *  -ss [filename] = write list of species to file (BNGL format) at the end of
 * simulation. This list is not guaranteed to be canonical. Filename argument is
 *                     optional (defaults to [model]_nf.species).
 *
 *  \section devel_sec Developers
 * To begin developing and extending NFsim, the best place to start looking is
 * in the src/NFtest/simple_system directory. Here you'll find two files,
 * simple_system.hh and simple_system.cpp.  Together, this code specifies a
 * simple enzymatic type reaction that is completely hard coded.  This will give
 * you an idea of the basic classes and functions used to define, initialize,
 * run, and output a simulation.  From there, you can dive into the specific
 * classes and functions that you need to work with.  Details about how to run
 * the simple_system example are given in these files.
 *
 * All of the other main classes are defined in the NFcore namespace and are
 * found in the NFcore directory and the NFreactions directory.  The NFcore
 * directory contains the basic structure of the simulation engine while the
 * NFreactions directory contains the classes associated with actually executing
 * rules and transforming molecules.  NFinput contains what's needed for the xml
 * parser (built using the TinyXML package) and the command line parser.  NFutil
 * also contains a nice implementation of the Mersenne Twister random number
 * generator which should be used for all random number generation in NFsim.
 * NFoutput is more sparse as it deals only with handling the more complicated
 * output required of groups and complexes.  (Basic outputting is handled easily
 * with the System and Observable classes in the NFcore namespace).
 *
 * Another note for developers: class functions and member variables are
 * generally well commented in the header file in which they are declared.  So
 * if you are lost in some source file, and you think there aren't any comments,
 * be sure to check the header file before you ask for help.
 *
 *  \section author_sec Authors & Acknowledgments
 * The original NFsim code was written and developed by Michael Sneddon with
 * help from James Faeder and Thierry Emonet.  James Faeder wrote the original
 * extended BioNetGen code that can output XML encodings of the BNGL and
 * contains the functional rate law syntax.  Justin Hogg developed the
 * capability to simulate population objects (a single object that aggregates
 * multiple identical molecular agents) and significantly improved NFsim's
 * internal local function handling.
 *
 * A number of other people have helped in getting NFsim to where it is today,
 * either by aiding in the concepts of the design, testing the implementation,
 * adding some features to the code, or by suggesting improvements.
 *
...
exec(const map<string, string>& argMap, bool& verbose, bool& parsed)
{
    ...
    if (argMap.empty()) {
        cout << endl
             << "\tNo parameters given, so I won't do anything." << endl;
    ...
    // Handle the case of running a predefined test
        ...
        if (!test.empty()) {
            ...
            if (!foundATest) {
                cout << "  That test could not be identified!!  Skipping!"
                     << endl;
            }
    ...
    if (argMap.find("logo") != argMap.end() ||
        argMap.find("version") != argMap.end()) {
        cout << endl << endl;
...
main(int argc, char* argv[])
{
    ...
    string versionNumber{ NFSIM_VERSION };
    ...
    if (argMap.find("xml") != argMap.end()) {
        ...
        if (!filename.empty()) {
            ...
            if (argMap.find("bscb") != argMap.end()) {
                if (verbose)
                    cout << "  Blocking same complex binding...\n";
            ...
            if (argMap.find("gml") != argMap.end()) {
                globalMoleculeLimit =
                  NFinput::parseAsInt(argMap, "gml", globalMoleculeLimit);
            }
            ...
            System* s =
              NFinput::initializeFromXML(filename,
                                         cb,
                                         globalMoleculeLimit,
                                         verbose,
            ...
            if (s != NULL) {
                ...
                // If requested, be sure to output the values of global
                // functions
                if (argMap.find("ogf") != argMap.end()) {
                    s->turnOnGlobalFuncOut();
                    if (verbose)
                        cout << "\tGlobal function output (-ogf) flag detected."
                             << endl
                             << endl;
                }
                ...
                if (argMap.find("dump") != argMap.end()) {
                    if (!NFinput::createSystemDumper(
                          argMap.find("dump")->second, s, verbose)) {
                        cout << endl
                             << endl
                             << "Error when creating system dump outputters.  "
                                "Quitting."
                             << endl;
                        delete s;
                        return 0;
                ...
                if (argMap.find("utl") != argMap.end()) {
                    ...
                    if (verbose)
                        cout << "\tUniversal Traversal Limit (UTL) set "
                                "manually to: "
                             << utl << endl
                             << endl;
                } else {
                ...
                if (verbose) {
                    ...
                    if (s->getEvaluateComplexScopedLocalFunctions()) {
                        cout << "\tComplex-scoped local function evaluation is "
                                "enabled."
                             << endl
                             << endl;
                    } else {
                ...
                if (argMap.find("oec") != argMap.end()) {
                    ...
                    if (verbose)
                        cout << "\tEvent counter output (-oec) flag detected."
                             << endl
                             << endl;
                }
                ...
                if (argMap.find("b") != argMap.end()) {
                    ...
                    if (verbose)
                        cout
                          << "\tStandard output is switched to binary format."
                          << endl
                          << endl;
                }
                ...
                if (argMap.find("rtag") != argMap.end()) {
                    vector<int> sequence;
                    NFinput::parseAsCommaSeparatedSequence(
                      argMap, "rtag", sequence);

                    if (verbose) {
                        cout
                          << "\tTagging reactions by id (from the -rtag flag):";
                        for (unsigned int k = 0; k < sequence.size(); k++)
                            cout << " " << sequence.at(k);
                        cout << endl;
                    }
                    for (unsigned int k = 0; k < sequence.size(); k++)
                        s->tagReaction(sequence.at(k));
                }
                ...
                if (argMap.find("o") != argMap.end()) {
                    ...
                    if (s->isOutputtingBinary()) {
                        ...
                        if (verbose) {
                            cout << "\tStandard output will be written to: "
                                 << s->getName() + "_nf.dat" << endl
                                 << endl;
                        }
                    } else {
                        s->registerOutputFileLocation(s->getName() +
                                                      "_nf.gdat");
                        s->outputAllObservableNames();
                        if (verbose)
                            cout << "\tStandard output will be written to: "
                                 << s->getName() + "_nf.gdat" << endl
                                 << endl;
                    }
                }
                ...
                if (argMap.find("notf") != argMap.end()) {
                    ...
                    if (verbose)
                        cout << "\tOn-the-fly observables is turned on "
                                "(detected -notf flag)."
                             << endl
                             << endl;
                }

                // Finally, return the system if we made it here without
                // problems
                return s;
            } else {
                cout << "Couldn't create a system from your XML file.  I don't "
                        "know what you did."
                     << endl;
                return 0;
            }
        } else {
            cout << "-xml flag given, but no file was specified, so no system "
                    "was created."
                 << endl;
        }
    } else {
        cout << "Couldn't create a system from your XML file.  No -xml "
                "[filename] flag given."
             << endl;
    }
    return 0;
...
runFromArgs(System* s, map<string, string> argMap, bool verbose)
{
    ...
    // If requested, walk through the simulation instead of running the
    // simulation
    if (argMap.find("walk") != argMap.end()) {
        ...
        cout << endl
             << endl
             << endl
             << "Equilibrating for :" << eqTime << "s.  Please wait." << endl
             << endl;
...
printLogo(int indent, string version)
{
    ...
    if (space < 0) {
        cout << "\n\nCome on!!! you don't even know how to print out the NFsim "
                "logo!"
             << endl;
...
printHelp(string version)
{
    cout << "To run NFsim at the command prompt, use flags to specify what you "
            "want"
         << endl;
    cout
      << "to do.  Flags are given in this format in any order: \"-flagName\"."
      << endl;
    cout
      << "Some of the flags require an additional parameter.  For instance, the"
      << endl;
    cout << "-xml flag requires the filename of the xml file.  The format "
            "would look"
         << endl;
    cout << "something like: \"-xml modelFile.xml\".  Simulation output is "
            "dumped to"
         << endl;
    cout << "a file named: \"[modelName]_nf.gdat\" in the current directory by "
            "default."
         << endl;
    cout << "" << endl;
    cout << "Here is a list of most of the possible flags:" << endl;
    cout << "" << endl;
    cout << "  -help             well, you already know what this one does..."
         << endl;
    cout << "" << endl;
    cout << "  -xml [filename]   used to specify the input xml file to read.  "
            "the xml"
         << endl;
    cout << "                    file must be given directly after this flag."
         << endl;
    cout << "" << endl;
    cout << "  -rnf [filename]   used to specify an rnf script to execute."
         << endl;
    cout << "" << endl;
    cout << "  -o [filename]     used to specify the output file name." << endl;
    cout << "" << endl;
    cout << "  -sim [time]       used to specify the length (in seconds) of a "
            "simulation"
         << endl;
    cout << "                    when running an xml file.  Fractional seconds "
            "are valid."
         << endl;
    cout << "                    for instance, you could use: -sim 525.50"
         << endl;
    cout << "" << endl;
    cout << "  -eq [time]        used to specify the length (in seconds) to "
            "equilibrate the"
         << endl;
    cout << "                    system before running the simulation." << endl;
    cout << "" << endl;
    cout << "  -oSteps [steps]   used to specify the number of times "
            "throughout the"
         << endl;
    cout << "                    simulation that observables will be "
            "outputted.  Must"
         << endl;
    cout << "                    be an integer value.  Default is to output "
            "once per"
         << endl;
    cout << "                    simulation second." << endl;
    cout << "" << endl;
    cout << "  -v                specify verbose output to the console."
         << endl;
    cout << "" << endl;
    cout << "  -b                use this flag to tell NFsim to output in "
            "binary (not ascii)"
         << endl;
    cout << "" << endl;
    cout << "  -notf             tells NFsim to Not use On The Fly output.  "
            "Normally,"
         << endl;
    cout << "                    observables are computed On The Fly - that is "
            "they are"
         << endl;
    cout << "                    updated after every simulation step.  This is "
            "good if you"
         << endl;
    cout << "                    output frequently or have many molecules in "
            "your system."
         << endl;
    cout << "                    However, it can be faster to recompute "
            "observable counts"
         << endl;
    cout << "                    right before you output especially if you "
            "don't output"
         << endl;
    cout << "                    too often.  Use this flag to switch to "
            "recomputing at "
         << endl;
    cout << "                    every output step instead of using On The Fly "
            "output."
         << endl;
    cout << "" << endl;
    cout << "  -ogf              output the value of all global functions."
         << endl;
    cout << "" << endl;
    cout << "  -utl [integer]    sets the universal traversal limit" << endl;
    cout << "" << endl;
    cout << "  -nocslf           disable evaluation of complex-scoped local "
            "functions."
         << endl;
    cout << "                    This may reduce run-time for some models, but "
            "will lead"
         << endl;
    cout << "                    to erroneous results if complex-scoped local "
            "functions"
         << endl;
    cout << "                    are required." << endl;
    cout << "" << endl;
    cout << "  -test             used to specify a given preprogrammed test. "
            "Some tests"
         << endl;
    cout << "                    include \"tlbr\" and \"simple_system\".  "
            "Tests do not read"
         << endl;
    cout << "                    in other command line flags" << endl;
    cout << "" << endl;
    cout << "  -seed             used to specify the seed for the random "
            "number generator."
         << endl;
    cout << "                    This allows you to run the same simulation "
            "and get the"
         << endl;
    cout
      ...
      << endl;
    cout << "" << endl;
    cout << "  -logo             prints out the ascii NFsim logo, for your "
            "viewing pleasure."
         << endl;
    cout << "" << endl;
    cout << "" << endl;
