 ...
 *
 * All of the other main classes are defined in the NFcore namespace and are found in the
 * NFcore directory and the NFreactions directory.  The NFcore directory contains the
 * basic structure of the simulation engine while the NFreactions directory contains the
 * classes associated with actually executing rules and transforming molecules.  NFinput
 * contains what's needed for the xml parser (built using the TinyXML package) and the
 * command line parser.  NFutil also contains a nice implementation of the Mersenne
 * Twister random number generator which should be used for all random number generation
 * in NFsim.  NFoutput is more sparse as it deals only with handling the more complicated
 * output required of groups and complexes.  (Basic outputting is handled easily with the
 * System and Observable classes in the NFcore namespace).
 *
 * Another note for developers: class functions and member variables are generally well
 * commented in the header file in which they are declared.  So if you are lost in some
 * source file, and you think there aren't any comments, be sure to check the header file
 * before you ask for help.
 *
 *  \section author_sec Authors & Acknowledgments
 * The original NFsim code was written and developed by Michael Sneddon with help from
 * James Faeder and Thierry Emonet.  James Faeder wrote the original extended BioNetGen
 * code that can output XML encodings of the BNGL and contains the functional rate law
 * syntax.  Justin Hogg developed the capability to simulate population objects (a single
 * object that aggregates multiple identical molecular agents) and significantly improved
 * NFsim's internal local function handling.
 *
...
main(int argc, char* argv[])
{
    ...
    if (!parsed) {
        cout << "   NFsim could not identify what you wanted to do.\n   Try running "
                "NFsim with the -help flag for advice."
             << endl;
    }
...
initSystemFromFlags(map<string, string> argMap, bool verbose)
{
    ...
    if (argMap.find("xml") != argMap.end()) {
        ...
        if (!filename.empty()) {
            ...
            System* s = NFinput::initializeFromXML(filename,
              cb,
              globalMoleculeLimit,
              verbose,
            ...
            if (s != NULL) {
                ...
                if (argMap.find("ogf") != argMap.end()) {
                    ...
                    if (verbose)
                        cout << "\tGlobal function output (-ogf) flag detected." << endl
                             << endl;
                }
                ...
                if (argMap.find("dump") != argMap.end()) {
                    if (!NFinput::createSystemDumper(
                        ...
                        cout << endl
                             << endl
                             << "Error when creating system dump outputters.  Quitting."
                             << endl;
                        delete s;
                        return 0;
                    }
                ...
                if (argMap.find("utl") != argMap.end()) {
                    ...
                    if (verbose)
                        cout << "\tUniversal Traversal Limit (UTL) set manually to: "
                             << utl << endl
                             << endl;
                } else {
                    s->setUniversalTraversalLimit(suggestedTraveralLimit);
                    if (verbose)
                        cout << "\tUniversal Traversal Limit (UTL) set automatically to: "
                ...
                if (verbose) {
                    ...
                    if (s->getEvaluateComplexScopedLocalFunctions()) {
                        cout << "\tComplex-scoped local function evaluation is enabled."
                             << endl
                             << endl;
                    } else {
                        cout << "\tComplex-scoped local function evaluation is DISABLED!"
                             << endl
                             << endl;
                    }
                }
                ...
                if (argMap.find("oec") != argMap.end()) {
                    ...
                    if (verbose)
                        cout << "\tEvent counter output (-oec) flag detected." << endl
                             << endl;
                }
                ...
                if (argMap.find("b") != argMap.end()) {
                    s->setOutputToBinary();
                    if (verbose)
                        cout << "\tStandard output is switched to binary format." << endl
                             << endl;
                }
                ...
                if (argMap.find("notf") != argMap.end()) {
                    ...
                    if (verbose)
                        cout << "\tOn-the-fly observables is turned on (detected -notf "
                                "flag)."
                             << endl
                             << endl;
                }
                ...
                cout << "Couldn't create a system from your XML file.  I don't know what "
                        "you did."
                     << endl;
                return 0;
            }
            ...
            cout
              << "-xml flag given, but no file was specified, so no system was created."
              << endl;
        }
    } else {
        cout << "Couldn't create a system from your XML file.  No -xml [filename] flag "
                "given."
             << endl;
    }
    return 0;
...
printLogo(int indent, string version)
{
    ...
    if (space < 0) {
        cout << "\n\nCome on!!! you don't even know how to print out the NFsim logo!"
             << endl;
...
printHelp(string version)
{
    cout << "To run NFsim at the command prompt, use flags to specify what you want"
         << endl;
    cout << "to do.  Flags are given in this format in any order: \"-flagName\"." << endl;
    cout << "Some of the flags require an additional parameter.  For instance, the"
         << endl;
    cout << "-xml flag requires the filename of the xml file.  The format would look"
         << endl;
    cout << "something like: \"-xml modelFile.xml\".  Simulation output is dumped to"
         << endl;
    cout << "a file named: \"[modelName]_nf.gdat\" in the current directory by default."
         << endl;
    cout << "" << endl;
    ...
    cout << "" << endl;
    cout << "  -xml [filename]   used to specify the input xml file to read.  the xml"
         << endl;
    ...
    cout << "" << endl;
    cout << "  -sim [time]       used to specify the length (in seconds) of a simulation"
         << endl;
    cout << "                    when running an xml file.  Fractional seconds are valid."
         << endl;
    cout << "                    for instance, you could use: -sim 525.50" << endl;
    cout << "" << endl;
    cout
      << "  -eq [time]        used to specify the length (in seconds) to equilibrate the"
      << endl;
    cout << "                    system before running the simulation." << endl;
    cout << "" << endl;
    cout << "  -oSteps [steps]   used to specify the number of times throughout the"
         << endl;
    cout << "                    simulation that observables will be outputted.  Must"
         << endl;
    cout << "                    be an integer value.  Default is to output once per"
         << endl;
    ...
    cout << "" << endl;
    cout
      << "  -b                use this flag to tell NFsim to output in binary (not ascii)"
      << endl;
    cout << "" << endl;
    cout << "  -notf             tells NFsim to Not use On The Fly output.  Normally,"
         << endl;
    cout << "                    observables are computed On The Fly - that is they are"
         << endl;
    cout
      << "                    updated after every simulation step.  This is good if you"
      << endl;
    cout << "                    output frequently or have many molecules in your system."
         << endl;
    cout << "                    However, it can be faster to recompute observable counts"
         << endl;
    cout << "                    right before you output especially if you don't output"
         << endl;
    cout << "                    too often.  Use this flag to switch to recomputing at "
         << endl;
    cout << "                    every output step instead of using On The Fly output."
         << endl;
    cout << "" << endl;
    ...
    cout << "" << endl;
    cout << "  -nocslf           disable evaluation of complex-scoped local functions."
         << endl;
    cout << "                    This may reduce run-time for some models, but will lead"
         << endl;
    cout << "                    to erroneous results if complex-scoped local functions"
         << endl;
    cout << "                    are required." << endl;
    cout << "" << endl;
    cout << "  -test             used to specify a given preprogrammed test. Some tests"
         << endl;
    cout
    ...
    cout << "" << endl;
    cout
      << "  -seed             used to specify the seed for the random number generator."
      << endl;
    cout << "                    This allows you to run the same simulation and get the"
         << endl;
    cout << "                    exact same results perhaps to compare performance"
         << endl;
    cout << "" << endl;
    cout
      << "  -logo             prints out the ascii NFsim logo, for your viewing pleasure."
      << endl;
    cout << "" << endl;
    cout << "" << endl;
}
